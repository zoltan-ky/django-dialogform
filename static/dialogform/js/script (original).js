...
class IFrameDDialog {
    ...
    async create_dialog(htmlText, dialog_container) {
        ...
        dialog.classList = 'dialogform-dialog';
    ...
    async show(dialog) {
        ...
        var result = await new Promise((laid_out) => {
            ...
            this.iframe.contentWindow.addEventListener(
                // Here we use the 'focus' event on the window
                'focus', event => laid_out(true),{ once: true });
        });
        iframe.style.height = this.iframe.contentDocument.body.scrollHeight + 'px';        
        iframe.style.width =  this.iframe.contentDocument.body.scrollWidth + 'px';
        if (!(this.form = this.iframe.contentDocument.querySelector('.dialogform'))) {
...
class DDialog extends IFrameDDialog {
    ...
    async show(dialog) {
        // (hmmm..., where's the 'layout- done' event? :-( )
        var result = await new Promise((laid_out) => {
            // 'focusin' event listened to by Element
            dialog.addEventListener('focusin', event => laid_out(true),{ once: true });
            dialog.addEventListener('focus',   event => laid_out(true),{ once: true });
            dialog.show();
        });
